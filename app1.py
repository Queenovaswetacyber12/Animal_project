...
<style>
    ...
    /* Premium Sidebar UI Customization */
    [data-testid="stSidebar"] {
        background: linear-gradient(180deg, #0f5132 0%, #198754 100%) !important;
    }
    [data-testid="stSidebar"] * {
...
with st.sidebar:
    st.markdown("<h2 style='text-align: center; margin-bottom: 1rem;'>🐄 Livestock Pro</h2>", unsafe_allow_html=True)
    
    menu = st.radio(
        "অপশন নির্বাচন করুন / Navigation Matrix",
        [
            "🏠 হোম | Home",
...
if menu == "🏠 হোম | Home":
    st.markdown("<h1 style='color: #111827;'>Livestock AI Assistant</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #4b5563; font-size: 1.1rem;'>Calculate dynamic live weight, market assessments and leverage expert medical machine diagnostics.</p>", unsafe_allow_html=True)
    ...
    st.markdown("### System Diagnostics Capabilities")
...
# =========================================
    ...
    <div class='pro-card'>
        ...
        <p><b>Heart Girth:</b> Measure the global chest circumference precisely behind the front forelegs.</p>
        <p><b>Body Length:</b> Measure in a straight linear projection from the point of the shoulder down to the pin bone.</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Grid system matching target metrics
...
# =========================================
    ...
    if st.button("Run Analytical Computation"):
        if animal == "Goat":
            # Universal Caprine Mathematical Formula
            base_lbs = (girth * girth * length) / 630
            weight_kg = base_lbs * 0.453592 * 2  # Adjusted factor array
            
            if weight_kg < 20: rate = 450
            elif weight_kg < 40: rate = 500
            elif weight_kg < 60: rate = 550
            else: rate = 600
            ...
            weight_kg = base_lbs * 0.453592
            rate = 0 # Variable dynamic regional pricing
...
elif menu == "🐄 スマート পশুপালন | Smart Animal Husbandry":
    st.title("🐄 স্মার্ট পশুপালন | Smart Husbandry Strategy")
    
    st.markdown("""
    <div class='pro-card'>
        <h3>🏠 ১. আধুনিক খামার পরিকাঠামো</h3>
        <p>বায়ু চলাচল ব্যবস্থার আধুনিকায়ন নিশ্চিত করুন। জীবাণুনাশক প্রয়োগ চক্র বজায় রাখুন।</p>
    </div>
    <div class='pro-card'>
                    ...
                    response = requests.post(
                        ...
                        json={
                            ...
                            "messages": [
                                ...
                                {"role": "user", "content": f"Animal Type: {selected_animal}. Symptoms: {symptoms}. Provide provisional dynamic analysis."}
                            ]
                        }
                    )
...
# =========================================
    ...
    st.info("💡 দ্রষ্টব্য: আবহাওয়া ও অঞ্চলভেদে ভ্যাকসিনের সময়সূচী পরিবর্তন হতে পারে।")
...
# =========================================
    ...
    if user_query:
        ...
        with st.chat_message("assistant"):
            if not API_KEY:
                st.error("API Key Configuration Error. Please contact app administrator.")
                st.stop()
                ...
                response = requests.post(
                    ...
                    json={
                        ...
                        "messages": [
                            ...
                            *st.session_state.vet_chat_messages[-5:] # Balanced historical optimization
                        ],
                        "temperature": 0.3
                    }
                )
...
# =========================================
    ...
    st.markdown("""
    <div class='pro-card'>
        <h3>Livestock AI Assistant - v2.1.0 (Pro Release)</h3>
        <p>This enterprise engine integrates deep precision analytics algorithms with low-latency LLM inference architectures to service remote farming infrastructure via web or native container app architectures.</p>
        <hr>
        <p><b>Target Form Factor:</b> Adaptive Web Framework / Cross-Platform Android APK / Desktop Matrix.</p>
    </div>
    """, unsafe_allow_html=True)
